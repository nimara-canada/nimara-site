...
export function TemplateCard({ slug, badge, title, subtitle }: TemplateCardProps) {
  return (
    <Link
      to={`/resources/${slug}`}
      className="group block rounded-2xl bg-card p-5 shadow-soft ring-1 ring-black/5 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--nimara-purple))] focus-visible:ring-offset-2"
    >
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="mb-2">
            <span 
              ...
              aria-label={badge === "NEWEST" ? "Newest template" : "Recommended template"}
            >
              {badge}
            </span>
          </div>
          ...
          <span
            className="inline-flex items-center gap-2 text-sm font-semibold self-start min-h-[44px] min-w-[44px] -mx-2 px-2 py-2 rounded-lg transition-colors hover:bg-black/5"
