...
const YourJourney: React.FC = () => {
  ...
  const addOnsInView = useInView(addOnsRef, {
    once: true,
    margin: "-50px"
  });
        ...
        <div ref={sectionRef} className="mb-24">
          <div className="hidden lg:block relative">
            ...
            <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className="relative z-10">
              {steps.map((step, index) => <motion.div key={step.number} variants={itemVariants} className={`flex items-center ${index % 2 === 0 ? 'justify-start' : 'justify-end'} mb-24 last:mb-0`}>
          ...
          <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className="lg:hidden relative">
            <motion.div initial={{
            scaleY: 0
          }} animate={isInView ? {
            scaleY: 1
            ...
            <div className="relative z-10">
              {steps.map(step => <motion.div key={step.number} variants={itemVariants} className="flex gap-6 mb-12 last:mb-0">
        ...
        <div ref={addOnsRef} className="relative">
          ...
          <motion.div initial="hidden" animate={addOnsInView ? "visible" : "hidden"} variants={containerVariants} className="pt-16 pb-8 px-8 bg-muted/50 rounded-3xl">
            ...
            <motion.div variants={containerVariants} className="grid md:grid-cols-2 gap-6">
              {addOns.map(addon => <motion.div key={addon.title} variants={itemVariants} whileHover={{
              y: -8,
              transition: {
                duration: 0.2,
                ease: "easeOut"
