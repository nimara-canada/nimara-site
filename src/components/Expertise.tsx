...
const AccordionItem = ({ item, isOpen, onToggle, index }: AccordionItemProps) => {
  ...
  return (
    ...
    >
      <button
        onClick={onToggle}
        className="w-full p-5 flex items-start gap-4 text-left focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-inset rounded-xl"
      ...
      >
        ...
        >
          <Icon 
            size={22} 
            className={`transition-colors duration-300 ${isOpen ? 'text-white' : 'text-slate-500'}`} 
          />
        </motion.div>
        ...
        >
          {isOpen ? (
            ...
            <Plus size={16} className="text-slate-500" strokeWidth={2.5} />
          )}
        </motion.div>
      </button>
        ...
        {isOpen && (
          ...
          >
            <div className="px-5 pb-5 pt-0">
              <div className="ml-15 pl-4 border-l-2 border-slate-200">
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div 
          ...
          variants={fadeInUp}
        >
          <span className="text-xs font-semibold tracking-widest text-violet-600 uppercase mb-4 block">
            Our Domains
          </span>
          
          <h2
            id="expertise-heading"
            className="text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 leading-tight tracking-tight mb-6"
          >
        ...
        >
          <button
            type="button"
            onClick={expandAll}
            className="text-sm font-medium text-slate-600 hover:text-violet-600 transition-colors focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 rounded px-3 py-1.5"
          >
            Expand all
          </button>
        ...
        >
          <div className="inline-flex flex-col sm:flex-row items-center gap-4 p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
            <div className="flex -space-x-2" aria-hidden="true">
              {expertiseItems.slice(0, 4).map((item, i) => {
                ...
                return (
                  <div 
                    key={i}
                    className={`w-8 h-8 rounded-full ${colors.bg} flex items-center justify-center ring-2 ring-white`}
                  >
            ...
            <p className="text-sm text-slate-600">
              <span className="font-medium text-slate-900">All 6 domains</span> covered in our Health Check
            </p>
            <a
              href="#contact"
              className="inline-flex items-center gap-2 bg-violet-600 hover:bg-violet-500 text-white px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2"
            >
