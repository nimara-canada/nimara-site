...
const NimaraHeroPremium = () => {
  ...
  // Progress bar data
  const progressBars = [
    { domain: "Board", score: 85, color: "mint" },
    { domain: "Money & Grants", score: 72, color: "gradient" },
    { domain: "Programs", score: 80, color: "mint" },
    { domain: "Tools & Files", score: 68, color: "gradient" },
  ];
  ...
  return (
    <section 
      ref={heroRef} 
      id="hero" 
      aria-label="Hero section - Build systems that make your nonprofit Proof-Ready"
    ...
    >
        ...
        <div className="flex-1 w-full max-w-7xl mx-auto px-6 lg:px-12 py-20 lg:py-28 flex flex-col">
          ...
          <div className="flex-1 grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div>
              ...
              {/* Subheadline */}
              <p 
                style={{ ...revealStyle(200), color: '#A0A0A0' }} 
              ...
              <div style={revealStyle(300)} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                ...
                <Link 
                  ...
                  className="inline-flex items-center gap-3 px-8 py-4 font-semibold rounded-lg select-none transition-all duration-200 hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                  style={{ 
              ...
              <div style={revealStyle(500)} className="mt-12 lg:hidden">
                <MobileDashboard isLoaded={isLoaded} progressBars={progressBars} />
              </div>
            </div>

            {/* Right - Dashboard Visual (Desktop) */}
          ...
          <div 
            ...
            className="mt-16 md:mt-20 pt-10 md:pt-12 border-t"
            role="region" 
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-4 text-center">
              <div className="relative sm:border-r sm:border-white/10">
                ...
                <span className="text-xs uppercase tracking-[0.2em]" style={{ color: '#A0A0A0' }}>
                  CAPACITY INSTALL
                </span>
              </div>
              <div className="relative sm:border-r sm:border-white/10">
      ...
      >
        <span 
          style={{ ...revealStyle(800), color: '#A0A0A0' }} 
...
interface DashboardProps {
  ...
  progressBars: Array<{ domain: string; score: number; color: string }>;
}

const DesktopDashboard = ({ isLoaded, revealStyle, progressBars }: DashboardProps) => {
  return (
    <div className="relative py-4">
      {/* Main Dashboard Card */}
      <div 
        style={{ ...revealStyle(400), backgroundColor: '#1E2130' }} 
        ...
        >
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.15em] mb-2" style={{ color: '#A0A0A0' }}>
                  OVERALL SCORE
                </p>
                <div className="flex items-baseline gap-1">
                  <span className="text-5xl font-bold text-white">78</span>
                  <span className="text-xl" style={{ color: '#A0A0A0' }}>/100</span>
                </div>
              </div>
              <div 
                className="px-4 py-2 rounded-lg text-xs font-semibold"
                style={{ backgroundColor: '#ACFCE3', color: '#0F1219' }}
              >
                Tier 3 — Strong
              </div>
            </div>
              ...
              {progressBars.map((item, index) => (
                <div key={item.domain} className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span style={{ color: '#A0A0A0' }}>{item.score}%</span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
                    <div 
                      ...
                      style={{
                        ...
                        background: item.color === 'mint' 
                          ? '#ACFCE3' 
                          : 'linear-gradient(90deg, #6945D8, #ACFCE3)',
      ...
      >
        ...
        <div className="space-y-2.5">
          {[
            { label: "Board minutes template", color: "#28C840" },
            { label: "Month-end routine", color: "#FEBC2E" },
            { label: "Role clarity docs", color: "#FEBC2E" },
          ].map((fix) => (
            <div key={fix.label} className="flex items-center gap-2 text-xs text-gray-600">
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: fix.color }} />
              {fix.label}
            </div>
          ))}
        </div>
      ...
      <div 
        ...
        className="absolute -left-4 bottom-8 z-10 bg-white rounded-xl shadow-2xl px-4 py-3"
      >
...
const MobileDashboard = ({ isLoaded, progressBars }: MobileDashboardProps) => {
  return (
    <div 
      className="relative rounded-xl overflow-hidden border"
      style={{ backgroundColor: '#1E2130', borderColor: 'rgba(255,255,255,0.1)' }}
    >
      ...
      >
        ...
        <span className="text-[10px] ml-auto" style={{ color: '#A0A0A0' }}>Health Score</span>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-sm" style={{ color: '#A0A0A0' }}>/100</span>
          </div>
          <span 
          ...
          {progressBars.map((item, i) => (
            <div key={item.domain} className="space-y-1">
              <div className="flex items-center justify-between text-[10px]">
                <span style={{ color: 'rgba(255,255,255,0.7)' }}>{item.domain}</span>
                <span style={{ color: '#A0A0A0' }}>{item.score}%</span>
              </div>
